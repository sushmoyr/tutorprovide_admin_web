...
import { useDesignations, useDeleteDesignation } from "@/hooks/use-designations";
import { extractApiError } from "@/lib/api/error";
import type { Designation, SortOrder } from "@/types";
...
export function DesignationsList() {
  ...
  const { data, isLoading, error } = useDesignations({ page, size: pageSize, sortBy, order });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMutation.mutateAsync(designationToDelete.id);
  ...
  if (designations.length === 0 && page === 0) {
    return (
  ...
  return (
    <>
              ...
              <TableRow key={designation.id} className="hover:bg-muted/50">
                ...
                <TableCell className="hidden sm:table-cell">
                  <span className="text-sm text-muted-foreground">
                    {format(new Date(designation.createdAt), "dd MMM yyyy")}
                  </span>
                </TableCell>
                <TableCell className="hidden sm:table-cell">
                  <span className="text-sm text-muted-foreground">
                    {format(new Date(designation.updatedAt), "dd MMM yyyy")}
                  </span>
                </TableCell>
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8"
                      onClick={() => setDesignationToEdit(designation)}
                    >
      ...
      <EditDesignationDialog
        ...
        onOpenChange={(open) => !open && setDesignationToEdit(null)}
      />
    </>
  );
}
