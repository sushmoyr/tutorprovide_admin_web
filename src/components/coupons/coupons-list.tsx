...
import type { Coupon, SortOrder } from "@/types";
...
export function CouponsList() {
  ...
  const { data, isLoading, error } = useCoupons({ page, size: pageSize, sortBy, order });
          ...
          <TableHeader>
            <TableRow className="bg-muted hover:bg-muted text-muted-foreground">
              ...
              <TableHead>
                ...
                >
                  {t("discountAmount")}
                  <SortIcon column="discountAmount" currentSort={sortBy} currentOrder={order} />
                </button>
              </TableHead>
              <TableHead>
                <button
                  onClick={() => handleSort("maxUsePerUser")}
                  className="inline-flex items-center font-medium"
                >
                  {t("maxUsePerUser")}
                  <SortIcon column="maxUsePerUser" currentSort={sortBy} currentOrder={order} />
                </button>
              </TableHead>
              <TableHead>
                <button
                  onClick={() => handleSort("maxUser")}
                  className="inline-flex items-center font-medium"
                >
                  {t("maxUser")}
                  <SortIcon column="maxUser" currentSort={sortBy} currentOrder={order} />
                </button>
              </TableHead>
              <TableHead>
                <button
                ...
                >
                  {t("usedCount")}
                  <SortIcon column="usedCount" currentSort={sortBy} currentOrder={order} />
                </button>
              </TableHead>
              <TableHead className="hidden sm:table-cell">
                ...
                >
                  {t("expiryDate")}
                  <SortIcon column="expiryDate" currentSort={sortBy} currentOrder={order} />
                </button>
              </TableHead>
          ...
          <TableBody>
            {coupons.map((coupon) => (
              <TableRow key={coupon.id} className="hover:bg-muted/50">
                ...
                <TableCell>
                  <span className="text-sm tabular-nums">{coupon.maxUsePerUser}</span>
                </TableCell>
                <TableCell>
                  <span className="text-sm tabular-nums">{coupon.maxUser}</span>
                </TableCell>
                <TableCell>
                  <span className="text-sm tabular-nums">{coupon.usedCount}</span>
                </TableCell>
